/* ============================================================
   Ballpit —— React Bits 组件原生移植版
   three.js 球池物理动效（重力 / 碰撞 / 跟随光标），透明度 50%
   依赖：assets/js/three.min.mjs（本地打包的 three，离线可用）
   使用：<div id="ballpitFooter" class="ballpit-host"></div>
   ============================================================ */
import {
  Vector3, MeshPhysicalMaterial, InstancedMesh, Timer, AmbientLight,
  ...
  PointLight, ACESFilmicToneMapping, Plane, Raycaster, RoomEnvironment
} from './three.min.mjs';

/* ====== 渲染器框架（three 官方 createRenderer 逻辑移植） ====== */
class x {
  #e;
  canvas;
  camera;
...
const X = {
  count: 120,
  colors: [0x274BA0, 0x5E89E4, 0x8FB0F5, 0x7FD8E8, 0xFFFFFF],
...
(function initBallpit() {
  /* 移动端禁用：WebGL 动画在手机上消耗大量 GPU，导致间歇性卡死 */
  if (window.innerWidth <= 768) return;
  const host = document.getElementById('ballpitFooter');
